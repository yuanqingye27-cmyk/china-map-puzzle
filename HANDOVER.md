...
**23 张天地图官方数据地图（中国 → 四川 → 21 个市州）已交付并锁定 v1.0.0，925 项测试全绿；
238 条资料卡的面积已全部补齐（脚本从官方边界几何算出，全覆盖）；
**四川 21 个市州已全部跑过**：区县文案三要素完整 **81/183（44%）**、有内容 **155/183（85%）**；
...
今天我想做：<填：把四川剩下 7 个市跑完 / 接入某个新省 / 优化已完成市的文案质量>
```
...
## 1.5 进度快照（2026-09-17 收尾 · 下次从这里接）

**一句话**：面积全覆盖；**四川 21 个市州已全部动过**，
区县文案 **三要素完整 81/183（44%）、有内容 155/183（85%）**；
流水线已完全成型，剩的就是**按批跑**。
...
| 状态 | 市 |
| --- | --- |
| ✅ 整市做完 | 成都 20/20、乐山 11/11 |
| 🔶 已跑过、有留空 | 雅安 6/8、广元 5/7、自贡 5/6、达州 4/7、凉山 4/17、阿坝 3/13、巴中 3/5、宜宾 3/10、德阳 2/6、甘孜 2/18、广安 2/6、绵阳 2/9、南充 2/9、资阳 2/3、泸州 1/7、眉山 1/6、内江 1/5、攀枝花 1/5、遂宁 1/5 |
| ⬜ 完全未动 | **无**（21 个市州全部跑过至少一轮） |

> 注：数字是"三要素完整"的条数。留空是**有意的**——素材里没有可核实内容时按"宁缺毋假"留空。

**下一步两个方向（二选一）：**

```bash
# 方向 A：把四川的留空补起来（102 条待补）
node tools/status.js --verbose            # 看哪些市还有缺口
# 对缺口大的市（凉山 13、甘孜 16、阿坝 10）重跑一轮：
node tools/facts-batch.js --map=<市> --delay=5000 --prompt   # 缓存命中会很快
node tools/facts-verify.js --map=<市> --in=out/verify-<市>.json --apply

# 方向 B：开始接四川以外的省（例如广东 440000）
# ① 在 tools/lib/slugs.js 的 CITY_SLUGS 补该省地级市拼音
node tools/batch-add-maps.js --parent=440000          # 接一个省
node tools/area-from-geo.js --parent=440000 --write   # 面积（脚本，秒级）
# ② 然后按 §3 P0 的流水线补文案
```

**下一次开工前先跑这三条**（10 秒，确认没坏）：

```bash
node tools/test-area-pick.js 2>&1 | tail -2        # 面积抽取器回归（21 项）
node tools/test-facts-verify.js 2>&1 | tail -2     # 防编造守门人自测（20 项）
node tools/area-from-geo.js --check 2>&1 | tail -2 # 面积算法锚点（5 项）
```

**⚠ 抓取限流（实测过，务必看）：**
- 抓取必须**串行**，而且"总数 × 间隔"要控制住：**每批 3~5 个、间隔 ≥5 s、批之间再 sleep 25 s**
- 被限流时**不要重试硬撞**（重试等于加码）；等 10~30 分钟，或改用 `--delay=0` 走缓存
- `facts-batch` **已支持增量合并**（按 adcode），分小批抓不会互相覆盖；
  `--fresh` 才是强制只保留本次结果
- 缓存落在 `.cache/facts/`（当前 150+ 页）；`--delay=0` 全量重抽通常 <1 秒

**质量档位**（上一轮已确认按"机器档"跑）：`--apply` 自动落笔，
保证**真**（可追溯、非编造、面积准），但会有平淡内容（地理志/获奖名单类能过校验）。
之后可按 `out/facts-*.json` 挑值得优化的市重做。

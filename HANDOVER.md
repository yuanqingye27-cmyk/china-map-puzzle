...
**23 张天地图官方数据地图（中国 → 四川 → 21 个市州）已交付并锁定 v1.0.0，902 项测试全绿；
238 条资料卡的面积已全部补齐（脚本从官方边界几何算出，全覆盖）；文案已补 41 条
（成都 20 + 乐山 11 + 自贡 5 + 巴中 5 + 内江 3 + 攀枝花 4）；
...
3. 跑 node tools/e2e-test.js 2>&1 | tail -6      # 基线应为 902 通过 / 0 失败
...
node tools/e2e-test.js 2>&1 | tail -6         # 基线：902 通过 / 0 失败
...
- **流水线实测（一口气跑了 4 个市，共 17 条文案）**：
  | 市 | 结果 | 素材问题 |
  | --- | --- | --- |
  | 自贡 | 5 条（大安区素材空，保持占位） | — |
  | 巴中 | 5 条 | 子代理用记忆补了 2 处 landmark（被新检查抓出并改正） |
  | 内江 | 3 条（市中区/东兴区保持占位） | 市中区抓成同名条目（0.09 倍）；东兴区 404 |
  | 攀枝花 | 4 条（西区保持占位） | 西区抓成河南石龙区；米易/盐边只有河流面积句 |
- **三个质量提示（实测踩过）**：
  ...
  - **素材为空就保持占位**。宁缺毋假 —— 已按此处理 4 个区县（自贡大安区、
    内江东兴区/市中区、攀枝花西区）
...
node tools/e2e-test.js 2>&1 | tail -6                  # 全部测试（902）

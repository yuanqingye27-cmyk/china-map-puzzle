...
- **线上**：`https://map-puzzle-89v-1v6.pages.dev`（Cloudflare Pages）
  - ⚠️ **账号下有两个项目，域名只差一截，极易认错**：
    | 项目名 | 它的域名 | 内容 |
    | --- | --- | --- |
    | **`map-puzzle-89v`** | **`map-puzzle-89v-1v6.pages.dev`** | **正式站（部署到这里）** |
    | `map-puzzle` | `map-puzzle-89v.pages.dev` | 早期同名项目，**默认图还是成都**，已弃用 |
    曾因此把部署验证跑在弃用项目上，误判"部署了但没生效"（白查了半小时）。
    **判断依据永远是域名里的 `-1v6`。**
  - ⚠️ 部署后**必须验证打开的是自己的内容**：`wrangler pages deploy` 会同时给出
    一个带哈希的预览地址，而预览/正式可能内容不同。`tools/release.js` 会在部署后
    抓线上 `js/game.js` 比对 `DEFAULT_MAP`，不一致就报错
  - ⚠️ "撞名不报错"也是真的：`--project=map-puzzle` 会撞上一个叫 "World Puzzle"
    的商业站，命令成功但打开是别人的站。`release.js` 现在会拦这个名字
...
| 低端机帧率 | — | 桌面/平板/窄屏视口在 headless 下实测 **60fps、0 掉帧**，但 headless 跑在 Mac 上，**无法可靠复现低端 Android**。低帧率自动降级因此**刻意没做**（写了也无法验证）。要在真机上量一次再决定 |
| 镜头推近手感 | — | 幅度与时长已按实机反馈调过一轮（`zoomFitPx` 104→76、`easeInOutCubic`、320→460ms），需真人再试一次确认"丝滑" |

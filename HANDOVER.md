...
**一句话**：面积全覆盖；四川区县文案 **61/183 完整**（14 个市已动）；
流水线已完全成型，剩的就是**按批跑**。
...
| 状态 | 市（完整/总数） |
| --- | --- |
| ✅ 做完 | 成都 20/20、乐山 11/11 |
| 🔶 部分完成 | 雅安 6/8、广元 5/7、自贡 5/6、巴中 3/5、德阳 2/6、绵阳 2/9、资阳 2/3、泸州 1/7、眉山 1/6、内江 1/5、攀枝花 1/5、遂宁 1/5 |
| ⬜ **完全未动（7 个市 / 80 条）** | **甘孜 18、凉山 17、阿坝 13、宜宾 10、南充 9、达州 7、广安 6** |

**下次继续的第一条命令**（把四川收尾）：

```bash
# ① 批量抓取 + 生成委派指令（串行，避免限流）
for m in ganzi liangshan aba yibin nanchong dazhou guangan; do
  node tools/facts-batch.js --map=$m --delay=1800 --prompt 2>&1 | grep -E '成功|面积对不上'
done
# ② 用一次 workflow 并行委派这 7 个市（每个只回 3 行摘要）
# ③ 逐个落笔 + 自检
for m in ganzi liangshan aba yibin nanchong dazhou guangan; do
  node tools/facts-verify.js --map=$m --in=out/verify-$m.json --apply
done
node tools/test-maps.js 2>&1 | tail -3 && node tools/e2e-test.js 2>&1 | tail -8
```

> ⚠️ 甘孜/凉山/阿坝规模大（13~18 个县），建议**单独一轮**跑，别和别的市混在一批，
> 否则一个 workflow 里的子代理要读的素材太多、耗时拉长。

**还有一个待你拍板的决定**（上次停在这一点）：

质量档位有两档，当前默认用**机器档**（`--apply` 自动落笔）：
- **机器档**：保证"真"（可追溯、非编造、面积准），但会有平淡内容
  （"地理志"如降雨量/断层、"获奖名单"如入选某某百佳县——这些**能通过校验**）
- **人工档**：每个市我先看 `out/draft-<市>.md` 再过目，质量更接近成都/乐山，但慢得多

**建议**：先机器档把四川铺完，之后按 `out/facts-*.json` 挑值得优化的市重做。

**下一次开工前先跑这三条**（10 秒，确认没坏）：

```bash
node tools/test-area-pick.js 2>&1 | tail -2        # 面积抽取器回归（21 项）
node tools/test-facts-verify.js 2>&1 | tail -2     # 防编造守门人自测（18 项）

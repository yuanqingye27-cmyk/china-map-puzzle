...
| 测试 | **879 项断言**（89 城市回归 + 77 引擎功能 + 585 多地图冒烟/导航/窄屏 + 13 项 WKT→GeoJSON 单测 + 115 项地图包自洽）+ 1 项 CSS 静态检查，全部通过 |
...
- 一次跑三套 + 两项离线检查，末尾打汇总。正常输出是 **`合计：879 通过 / 0 失败`**（城市回归 89 + 引擎功能 77 + 多地图冒烟 585 + WKT 13 + 地图包自洽 115）
...
3. **九段线 / 境界线（未处理，合规遗留项）**：中国层的官方响应里有 **8 个 `境界线`
   （MultiLineString）要素被跳过** —— 引擎只认面（拼图块必须是面），按红线没动引擎。
   南海诸岛（九段线）就在这一层，**跳过意味着底图不画它**；对外发布的地图需要显示它。
   计划做法：在 `index.html` 里加一层静态 SVG 覆盖层（属于宿主层，不碰引擎）。
...
│   ├── test-maps.js               · 地图包自洽离线自检（115 项：adcode/关卡/资料/来源/key）

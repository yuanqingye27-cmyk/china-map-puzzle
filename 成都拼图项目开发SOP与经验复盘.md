...
| 测试 | **764 项断言**（89 城市回归 + 77 引擎功能 + 585 多地图冒烟/导航/窄屏 + 13 项 WKT→GeoJSON 离线单测）+ 1 项 CSS 静态检查，全部通过 |
...
- 一次跑三套 + 两项离线检查，末尾打汇总。正常输出是 **`合计：764 通过 / 0 失败`**（城市回归 89 + 引擎功能 77 + 多地图冒烟 585 + WKT 离线 13）
...
| 地图 | 数据源 | 审图号 | 可商用 |
| --- | --- | --- | --- |
| 全部 23 张（中国 / 四川 / 21 市州） | 阿里云 DataV.GeoAtlas | **无** | ❌ 开发期数据源，不可公开商用 |

页面上的合规声明读的是每张地图**自己带的来源元信息**（`MAP_GEO_META`），
所以现在页面上如实写着：

> 地图数据来源：阿里云 DataV.GeoAtlas（开发期数据源，未取得审图号，不可用于公开商用）
...
1. **审图号适用范围**：GS(2024)0650号 覆盖的是天地图发布的行政区划数据产品。
   "调用 API 得到的边界用于商业产品"是否同样受它覆盖，**请向天地图/测绘主管部门书面确认**，
   别默认成立 —— 这是整个任务的前提。
2. **边界精度**：API 返回的是为地图展示做的简化边界。拼图对边界形态很敏感，
   建议先用 `--only` 换 2~3 张亲眼比对再放量（见上面清单第 3 条）。
3. **相邻边界是否严丝合缝**：DataV 的相邻区县共用同一套顶点，所以拼块之间没有缝。
   天地图按区县逐个返回边界，**相邻块之间可能出现细缝或重叠**（引擎用同一份数据画凹槽和碎片，
   所以"能不能拼对"不受影响，但视觉上要接受）。
4. **服务条款与配额**：天地图服务要求署名，且商用可能有额外约定；免费配额不足以支撑大规模批量。
...
│   ├── test-wkt.js                · WKT → GeoJSON 的离线单测（13 项，不需要浏览器/Key）

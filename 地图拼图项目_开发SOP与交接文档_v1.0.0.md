...
#### 坑 #42：把文档放进"可丢弃的产物目录"，然后被自己的重建命令删掉

**现象**：给 Cloudflare Pages 写的部署说明 `deploy/README-部署到Cloudflare.md`
（含 1000 文件上限的两条解法、上传步骤、排查顺序，几百行），在我跑
"重建 deploy/" 的 `rm -rf deploy && mkdir ...` 之后**消失了**。

**根因**：两个决定各自都合理，合起来就是事故：

1. `deploy/` 是**构建产物**，所以进了 `.gitignore`（对，产物不该入库）
2. 文档放进 `deploy/`，方便"打开文件夹就看到怎么传"（看起来也对）

于是这份文档**从未被 git 记录过**，而重建命令的第一步就是删掉整个目录 ——
**没有任何地方留了副本**。

**修法**：把文档移到 `docs/部署到Cloudflare.md`（会被版本管理），
并在文首写明"为什么它不在 `deploy/` 里"。
同时在重建小节加一句规则：

> `deploy/` 与 `deploy-sichuan/` 都在 `.gitignore` 里，**它们是可丢弃的**。
> 所以**不要把任何只此一份的东西放进这两个文件夹**。

**教训（可迁移）**：
- **"产物目录"和"文档目录"不能混。** 判据很简单：这个文件被删掉之后，
  **能不能一键重建**？能 → 放产物目录；不能 → 放被版本管理的地方
- 写"删掉再重建"这类命令时，**先想一遍它会连带删掉什么**
  （`rm -rf` 之前先 `ls` 一眼，或者改用"只覆盖、不整目录删"的写法）
- 这条和坑 #39 是同一类错误的不同面：**#39 是"改了但没落进产物"，
  #42 是"只有产物里有"** —— 两边都是"数据放错了地方"

---

#### 坑 #43：窄屏横向溢出，真凶是 flex/grid 的「最小内容宽度」下限

**现象**：中国图在 390px 宽的手机上 `body.scrollWidth = 583`（横向能拖出半屏），
`.topbar` 自己只有 368px，但它的子项 `.brand` 和 `.mapbar` 都是 572px。

**排查过程（值得记的是方法，不是结论）**：
一开始我按直觉修了三处都**没用**：给 `.app` 加 `grid-template-columns: minmax(0,1fr)`、
给 `.brand` 加 `min-width: 0`、给文字加 `text-overflow: ellipsis`。

真正解决问题的是**把"是谁下不去"打出来**：在断言里输出祖先链上每一层的宽度
+ 关键计算样式（width / minWidth / flex / overflow），一眼看到
"`.topbar` 368 而子项 572" —— 才知道问题在**子项的最低宽度**，不在容器。

**根因**：flex/grid 子项的 `min-width` 默认是 `auto`，其下限等于**最小内容宽度**；
`align-items: stretch` 想把子项拉到容器宽，却拉不到比 min-content 更窄。
只要链路上有一环没设 `min-width: 0`，整页就被顶开。

**修法**：窄屏下把顶栏从 flex 改成 **block**。
block 子元素的宽度恒等于容器宽度，整套 min-content 规则直接绕过 ——
比在每个子项上逐个补 `min-width: 0` 稳得多（补漏一个就复发）。

**教训**：
- **溢出的排查顺序：先量"谁最宽、链路上每一层多宽"，再改 CSS。** 凭直觉改三处是浪费
- **让测试说出"是谁"** —— 断言不只报"溢出了 583"，还要报最宽的元素与祖先链宽度。
  这条我已经写进 `tools/modes-test.html` 的溢出诊断里，下次直接看输出
- 同类坑在 README §8 记过一次（flex 子项的 `min-width:auto`），
  这次是它在 grid 与 `align-items: stretch` 下的变体

---

#### 坑 #44：手写的"文件清单"必然漏 —— 改成从源头推导

**现象**：`tools/deploy-pack.js` 里手写了一份"外壳脚本清单"，
新加 `js/modes.js` 之后没想起来补进去 —— 产出的部署包**少一个脚本**，
页面不报错、只是静默降级（模式选择器变成空的）。

**根因**：清单是**第二份真相**。凡是"某处列了一遍、源头还有一遍"的地方，
早晚会不一致，而且不一致时**不会报错**。

**修法**：清单改成**从 `index.html` 推导** —— 它就是"运行时要加载哪些脚本"的
唯一真相。同时保留产物校验（断言 `index.html` 引用的每个文件都在产物里），
万一推导逻辑本身出了问题也能拦住。

**教训**：
- **不要维护第二份真相。** 能推导就不要手写
- 必须手写时，**配一条断言**去对照源头（比如"产物里必须有 index.html 引用的每个文件"）
- 这和坑 #39 / #42 是同一个母题的三次变体：
  **#39 数据没落进产物 · #42 数据只在产物里 · #44 清单漏了数据** ——
  都是"同一个事实存在两处，然后对不上"

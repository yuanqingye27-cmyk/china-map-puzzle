...
function pickArea(text) {
  ...
  const scored = sents.map((s) => {
    ...
    if (/分布面积|灌溉面积|集雨面积|流域面积|流城面积|汇水面积|占地面积|建筑面积|建成区|保护区|绿化|耕地|林地面|水域面积|库容|洪水位|网格|覆盖面积|灌面/.test(s)) score -= 8;
    if (/东西长|南北宽|海拔|东经|北纬|干流长|河道长|河长/.test(s)) score -= 1;
    /* 子区域面积："东部丘陵区，面积263.4平方千米，占全区幅员面积的40.65%"
     * 讲的是区内某一**片区**，不是全域。实测德阳旌阳区因此被误判（0.41 倍）。 */
    if (/占[^，。；]{0,12}?面积[^，。；]{0,4}?[\d.]+%/.test(s)) score -= 10;
    if (/(东部|西部|南部|北部|中部|东南部|西北部|东北部|西南部)?(丘陵|平坝|山地|河谷|平原|盆地)(区|地带)/.test(s)) score -= 4;
    /* "某个具体对象"的面积 —— 第五次踩坑（雅安批次）：
     *   雨城区「砂石集中开采区…面积24.67」/ 汉源县「万里工业园规划面积7.2」
     *   石棉县「蟹螺乡江坝村四组…面积24」/ 天全县「红星村坐落于…面积59.6」
     * 判据：句子里出现"园区/开发区/村/乡/镇/基地/景区/开采区/规划面积"等具体对象词，
     * 说明它讲的是一个点，不是整个行政区。 */
    if (/(工业园|产业园|开发区|开采区|矿区|园区|景区|自然保护区|基地|规划面积|建成区|新区|片区)/.test(s)) score -= 10;
    if (/(村|乡|镇|街道|社区|组)[^，。；]{0,6}(面积|坐落|位于)/.test(s)) score -= 10;
    if (/(村|乡|镇|街道|社区)(四组|三组|二组|一组)/.test(s)) score -= 10;
    /* 只讲海拔/座落而没说"全域面积"的，一律不可信 */
    if (/海拔[\d.]+米/.test(s) && !/(全区|全县|全市|辖区|政区|幅员|境域)面积/.test(s)) score -= 6;

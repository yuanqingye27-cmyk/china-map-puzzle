...
const normalize = (s) => String(s || '')
  ...
  .replace(/\\+/g, '')
  /* 抽取器按句切分时会留下"年份被切掉"的句首（实测「年8月7日，郊区更名为仁和区」
   * 「年，赵一曼诞辰110周年之际…」）。这类残缺句首与素材原文对不上，
   * 但内容是真的，所以在比对前把句首的孤立"年/年代"残片与标点一起去掉。 */
  .replace(/^[，。；、：""''（）()【】\s]*[0-9]*年代?[，。；、：]?/, '')
  .replace(/\s+/g, '')
  .replace(/[，。；、：""''（）()【】\[\]·—…!?！？,.:;"']/g, '')
  .trim();
...
function main() {
  ...
  payload.districts.forEach((d) => {
    ...
    if (src.collision) {
      /* collision 是**弱信号**（只是"全文没出现所属市名"）。
       * 实测误报：西昌市/会理市/会东县/喜德县的条目里压根没写"凉山"二字，
       * 但它们的面积与几何一致、地名也对 —— 不是撞车。
       * 真正的撞车由 **面积比值** 抓（内江市市中区 0.05 倍那种）。
       * 所以这里只提醒，不拦。 */
      warn(tag + '：素材里没出现所属市名（弱信号，不一定错）—— 注意别给它加所属市相关的描述');
    }

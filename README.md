...
一个用交互式动画认识中国行政区划的网页小游戏：把打乱的省 / 市 / 区县碎片拖回地图上的正确位置，拼对后会弹出该区域的面积、地标和冷知识。

**覆盖全国 363 张地图**：中国（34 个省级，按地理分 7 关）→ 各省级 → 各地级。零依赖、零外链资源、双击即玩。
...
> 目前已接入 **363 张地图**（中国 34 个省级 → 329 个地级，含港澳台）。详见[想接入新地图？](#想接入新地图)
...
**全部 363 张地图的边界数据来自国家地理信息公共服务平台（天地图）**，审图号 **GS(2024)0650号**。
页面上始终显示：

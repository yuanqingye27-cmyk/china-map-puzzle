...
(function () {
  ...
  /* ============================ 配置 ============================ */
  const MAP_WIDTH = 1000;        // 画布逻辑宽度
  const MAP_PADDING = 14;        // 画布留白
  const PIECE_MAX = 62;          // 碎片在托盘里的最大边（像素）
  const PIECE_MIN_SIDE = 15;     // 碎片最小边，防止细长条形看不清
  const DRAG_THRESHOLD = 5;      // 移动超过这个距离才算"拖拽"，否则算"点击"
  const SNAP_MS = 300;           // 吸附动画时长，要和 CSS 里的 transition 对上

  /* 每一关的主色调（HSL 色相），同关内再微调出层次 */
  /* 三关的主色相：银杏金 / 竹青 / 青瓷，对应当下"青绿竹韵"的色板 */
  const LEVEL_HUE = { core: 34, inner: 158, outer: 196 };

  /* ============================ 数据 ============================ */
  const GEO = window.CHENGDU_GEO;
  const { DISTRICT_INFO, LEVELS } = window.DistrictData;
  ...
  function colorOf(adcode) {
    ...
    const base = LEVEL_HUE[level.id] || 160;
    const hue = base + ((idx * 11) % 44) - 22;
    const light = 48 + ((idx * 7) % 16);
  ...
  /* ============================ 初始化 ============================ */
  function init() {
    if (!GEO || !window.GeoMap) {
      document.body.innerHTML =
        '<p style="padding:40px;color:#e7f2ec;font-family:sans-serif">' +
        '地图数据没加载出来，请确认 js/map-data.js 存在且没有被浏览器拦截。</p>';
      return;
    }
  ...
  const SAVE_KEY = 'chengdu-map-puzzle';
  ...
  const THEME_KEY = 'chengdu-map-theme';
  const THEMES = ['jade', 'ginkgo', 'shu'];

  function applyTheme(name, animate) {
    const theme = THEMES.indexOf(name) >= 0 ? name : THEMES[0];
  ...
  function initTheme() {
    ...
    // 这里再设一次是为了同步色点的选中态
    applyTheme(saved || document.documentElement.getAttribute('data-theme') || 'jade', false);
  ...
  const SOUND_KEY = 'chengdu-map-sound';
  let audioCtx = null;
  let soundOn = true;
  ...
  function buildMap() {
    ...
    // 底图：20 个区县全部画一遍浅色轮廓，给玩家一个"这是成都"的参照系
  ...
  /* ------------------------- 视图聚焦 -------------------------
   * 中心城区 5 个区加起来只占全市面积的 3%，如果整张成都一直铺在画布上，
   * 第一关的凹槽会挤成看不清的一小团。所以每一关都把视图推到该关区县的范围，
   * 切换关卡时用 viewBox 插值做出平滑推近/拉远的效果。
   * ------------------------------------------------------------ */
  ...
  function finishLevel() {
    ...
    setTimeout(() => {
      ...
      el.modalText.textContent = isLast
        ? '你已经把成都的 20 个区县全部放回了正确的位置。现在点地图上的任意区块，可以慢慢看每个地方的介绍。'
  ...
  const INTRO_KEY = 'chengdu-map-intro-seen';
  let introTimer = null;

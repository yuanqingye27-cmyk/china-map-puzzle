/* =====================================================================
 * 地图包 · suizhou · 资料与关卡【占位骨架，等待人工补全】
 * ---------------------------------------------------------------------
 * 路径：js/maps/china/hubei/suizhou.data.js
 ...
 * 《suizhou.geo.js》的官方边界几何计算得出（球面多边形面积，与 d3.geoArea 同公式），
 * 与拼图所用边界严格同源、可复现；属"几何计算值"，不等于官方公布的统计口径面积。
 * 重新生成：node tools/area-from-geo.js --only=suizhou --write
...
(function (global) {
  ...
  const DISTRICTS = {
    "421303": {
      area: 1424,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 曾都区
    "421321": {
      area: 5539,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 随县
    "421381": {
      area: 2643,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 广水市
  ...
  const LEVELS = [
    {
      ...
      short: '第一关',
      color: '#6bd039',
      blurb: '本图的下级行政区全部是其余下级行政区（3 个）。想按地理相邻或主题重排关卡，直接改这个文件里的 levels。',
      adcodes: [421303, 421321, 421381],
    },
  ];

  const MAP_DATA = (global.MAP_DATA = global.MAP_DATA || {});
  MAP_DATA['suizhou'] = { districts: DISTRICTS, levels: LEVELS };
})(window);

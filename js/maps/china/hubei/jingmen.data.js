/* =====================================================================
 * 地图包 · jingmen · 资料与关卡【占位骨架，等待人工补全】
 * ---------------------------------------------------------------------
 * 路径：js/maps/china/hubei/jingmen.data.js
 * 生成：node tools/add-map.js --adcode=420800 --name=jingmen --parent=hubei
 *
 ...
 * 《jingmen.geo.js》的官方边界几何计算得出（球面多边形面积，与 d3.geoArea 同公式），
 * 与拼图所用边界严格同源、可复现；属"几何计算值"，不等于官方公布的统计口径面积。
 * 重新生成：node tools/area-from-geo.js --only=jingmen --write
...
(function (global) {
  ...
  const DISTRICTS = {
    "420802": {
      area: 1651,  // [geo-area] 依官方边界几何计算（km²）
    ...
    "420881": {
      area: 4423,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 钟祥市
    "420882": {
      area: 3522,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 京山市
  ...
  const LEVELS = [
    ...
    },
    {
      ...
      adcodes: [420881, 420882],
    },
  ];

  const MAP_DATA = (global.MAP_DATA = global.MAP_DATA || {});
  MAP_DATA['jingmen'] = { districts: DISTRICTS, levels: LEVELS };
})(window);

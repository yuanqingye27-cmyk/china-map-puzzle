/* =====================================================================
 * 地图包 · huizhou · 资料与关卡【占位骨架，等待人工补全】
 * ---------------------------------------------------------------------
 * 路径：js/maps/china/guangdong/huizhou.data.js
 * 生成：node tools/add-map.js --adcode=441300 --name=huizhou --parent=guangdong
 *
 ...
 * 《huizhou.geo.js》的官方边界几何计算得出（球面多边形面积，与 d3.geoArea 同公式），
 * 与拼图所用边界严格同源、可复现；属"几何计算值"，不等于官方公布的统计口径面积。
 * 重新生成：node tools/area-from-geo.js --only=huizhou --write
...
(function (global) {
  ...
  const DISTRICTS = {
    ...
    "441323": {
      area: 3499,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 惠东县
    "441324": {
      area: 2273,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 龙门县
  ...
  const LEVELS = [
    ...
    },
    {
      ...
      adcodes: [441322, 441323, 441324],
    },
  ];

  const MAP_DATA = (global.MAP_DATA = global.MAP_DATA || {});
  MAP_DATA['huizhou'] = { districts: DISTRICTS, levels: LEVELS };
})(window);

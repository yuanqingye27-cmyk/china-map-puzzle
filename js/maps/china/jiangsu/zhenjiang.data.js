/* =====================================================================
 * 地图包 · zhenjiang · 资料与关卡【占位骨架，等待人工补全】
 * ---------------------------------------------------------------------
 * 路径：js/maps/china/jiangsu/zhenjiang.data.js
 * 生成：node tools/add-map.js --adcode=321100 --name=zhenjiang --parent=jiangsu
 *
 ...
 * 《zhenjiang.geo.js》的官方边界几何计算得出（球面多边形面积，与 d3.geoArea 同公式），
 * 与拼图所用边界严格同源、可复现；属"几何计算值"，不等于官方公布的统计口径面积。
 * 重新生成：node tools/area-from-geo.js --only=zhenjiang --write
...
(function (global) {
  ...
  const DISTRICTS = {
    "321102": {
      area: 322,  // [geo-area] 依官方边界几何计算（km²）
  ...
  const LEVELS = [
    ...
    },
    {
      ...
      adcodes: [321181, 321182, 321183],
    },
  ];

  const MAP_DATA = (global.MAP_DATA = global.MAP_DATA || {});
  MAP_DATA['zhenjiang'] = { districts: DISTRICTS, levels: LEVELS };
})(window);

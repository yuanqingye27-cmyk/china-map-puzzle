/* =====================================================================
 * 地图包 · taizhou · 资料与关卡【占位骨架，等待人工补全】
 * ---------------------------------------------------------------------
 * 路径：js/maps/china/jiangsu/taizhou.data.js
 * 生成：node tools/add-map.js --adcode=321200 --name=taizhou --parent=jiangsu
 *
 ...
 * 《taizhou.geo.js》的官方边界几何计算得出（球面多边形面积，与 d3.geoArea 同公式），
 * 与拼图所用边界严格同源、可复现；属"几何计算值"，不等于官方公布的统计口径面积。
 * 重新生成：node tools/area-from-geo.js --only=taizhou --write
...
(function (global) {
  ...
  const DISTRICTS = {
    "321202": {
      area: 380,  // [geo-area] 依官方边界几何计算（km²）
    ...
    "321281": {
      area: 2396,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 兴化市
    "321282": {
      area: 657,  // [geo-area] 依官方边界几何计算（km²）
      landmark: '📖 资料收录中，欢迎参与共建',
      tagline: '📖 资料收录中，欢迎参与共建',
      funFact: '📖 资料收录中，欢迎参与共建',
    }, // 靖江市
  ...
  const LEVELS = [
    ...
    },
    {
      ...
      adcodes: [321281, 321282, 321283],
    },
  ];

  const MAP_DATA = (global.MAP_DATA = global.MAP_DATA || {});
  MAP_DATA['taizhou'] = { districts: DISTRICTS, levels: LEVELS };
})(window);
